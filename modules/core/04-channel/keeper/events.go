...
func emitSetTxHashEvent(ctx sdk.Context, channel types.Channel, packet exported.PacketI, hash []byte) {
	ctx.EventManager().EmitEvents(sdk.Events{
		sdk.NewEvent(
			types.EventTypeSetTxHash,
			sdk.NewAttribute(types.AttributeKeyDataHex, string(hash)),
			sdk.NewAttribute(types.AttributeKeyData, string(packet.GetData())), //nolint:staticcheck // DEPRECATED
			sdk.NewAttribute(types.AttributeKeyDataHex, hex.EncodeToString(packet.GetData())),
