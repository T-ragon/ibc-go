...
// in client state that must be the same across all valid aggrelite clients for the new chain.
// VerifyUpgrade will return an error if:
// - the upgradedClient is not a aggrelite ClientState
...
//   - any aggrelite chain specified parameter in upgraded client such as ChainID, UnbondingPeriod,
...
func (cs ClientState) VerifyUpgradeAndUpdateState(
	...
	// upgraded client state and consensus state must be IBC aggrelite client state and consensus state
	// this may be modified in the future to upgrade to a new IBC aggrelite type
	...
	if !ok {
		return errorsmod.Wrapf(clienttypes.ErrInvalidClientType, "upgraded client must be aggrelite client. expected: %T got: %T",
			&ClientState{}, upgradedClient)
	}
	tmUpgradeConsState, ok := upgradedConsState.(*ConsensusState)
	if !ok {
		return errorsmod.Wrapf(clienttypes.ErrInvalidConsensus, "upgraded consensus state must be aggrelite consensus state. expected %T, got: %T",
